...
suppressMessages({
  suppressWarnings({
    ...
    if (interactive()) {
      ...
      genome_size_path = "/path/to/your/data/genome_sizes.tsv"
    ...
    n_protein_per_genome = protein_to_genome %>% group_by(genome_id) %>% summarise(n_protein=n())
    
    # Read genome sizes and merge with protein counts
    genome_size = fread(genome_size_path) %>% left_join(n_protein_per_genome, by="genome_id") %>% replace(is.na(.),0)
    ...
    HMM_search_output = read_table(HMM_search_output_path, comment = "#", col_names = HMM_domtbl_out_colnames) %>%
      ...
      inner_join(protein_to_genome %>% select(protein_id, genome_id), by="protein_id") %>%
    ...
    MinProteins_score_based_prediction_f = function(category, HMM_filtered) {
      ...
      # keep only HMM hits passing the cutoff values 
      ...
      if (nrow(HMM_filtered_MinProteins) > 0) {
        ...
        # Extract MinProteins `Minimum number of conserved genes per genome` cutoff value for this P-P type
        ...
        check_profiles = HMM_filtered_MinProteins %>% 
          select(hmm_profile, genome_id)  %>% mutate(values = 1)  %>% 
          pivot_wider(names_from = genome_id, values_from = values)  %>%  
        ...
        TEST_info_by_genome_id_category = data.frame(n_hits_MinProteins = colSums(profile_presence_genome_id %>% select(-hmm_profile, -profile_scores_1, -profile_scores_2))) %>% 
          ...
          mutate(genome_id = rownames(.)) %>% left_join(genome_size, by="genome_id") %>% mutate(PP_category=category, cutoff_MinProteins = category_MinProteins_cutoff) %>%
          ...
          select(genome_id, size, n_protein, PP_category, cutoff_MinProteins,  n_hits_MinProteins, score_0_MinProteins, score_1_MinProteins, score_2_MinProteins)
        
      } else { # empty dataframe if no hits found
        
        TEST_info_by_genome_id_category = data.frame(genome_id = "NA", size = 0, n_protein = 0, PP_category = "NA", cutoff_MinProteins = 0, n_hits_MinProteins = 0, score_0_MinProteins = 0, score_1_MinProteins = 0, score_2_MinProteins = 0)[-1, ]
      }
    ...
    Composition_search_based_prediction_f = function(category, HMM_filtered) {
      ...
      # Extract tolerance to gaps threshold for this P-P type
      tolerance_threshold = ALL_cutoffs$composition_tol_thr[ALL_cutoffs$PP_category == category]
      
      # Filter compositions based on coverage >= 50%
      composition_comparison = Compositions_unique %>% inner_join(HMM_filtered %>% filter(cov_profile >= 0.5), by = "hmm_profile") %>% 
        group_by(Composition, genome_id) %>% mutate(number_hits = n()) %>% 
        
        filter(number_hits == composition_size | (number_hits >= composition_size - tolerance_threshold & number_hits >= 0.75*composition_size)) %>%
        group_by(genome_id) %>% arrange(desc(number_hits)) %>% slice(1) %>% ungroup()%>% 
        mutate(tol_thr = tolerance_threshold)
      
      if (nrow(composition_comparison) > 0) {
        
        TEST_info_by_genome_id_category = composition_comparison %>% left_join(genome_size, by="genome_id")  %>%
          mutate(size = as.numeric(size), n_protein = as.numeric(n_protein)) %>% 
          select(genome_id, size, n_protein, PP_category = PP_category.x, Composition, composition_size, tol_thr, n_hits_composition = number_hits)
        
      } else { # empty dataframe if no hits found
        
        TEST_info_by_genome_id_category = data.frame(genome_id = "NA", size = 0, n_protein = 0, PP_category = "NA", Composition = "NA", composition_size = 0, tol_thr = 0, n_hits_composition = 0)[-1, ]
    ...
    FINAL_SUMMARY_TABLE_ = full_join(SUMMARY_MinProteins_score_based_prediction_ALL, 
      ...
      mutate(size = case_when(!is.na(size.x) ~ size.x, T ~ size.y), n_protein = case_when(!is.na(n_protein.x) ~ n_protein.x, T ~ n_protein.y)) %>% 
      select(-size.x, -size.y, -n_protein.x, -n_protein.y) %>%
    ...
    FINAL_SUMMARY_TABLE = FINAL_SUMMARY_TABLE_ %>%
      rename(`Genome ID` = genome_id, `P-P type` = PP_category, `Genome size (bp)` = size, `Number of proteins` = n_protein,  `Tolerance to gaps` = tol_thr,
             `MinProteins cutoff` = cutoff_MinProteins, `MinProteins hits` = n_hits_MinProteins, `Composition size` = composition_size, `Composition hits` = n_hits_composition, 
    ...
    if (nrow(FINAL_PREDICTION_TABLE_) > 0) { 
      ...
      FINAL_PREDICTION_ = FINAL_PREDICTION_TABLE %>% 
        ...
        mutate(Confidence = case_when(size <= size_max & size >= size_min & Predicted_by == "MinProteins + Composition" ~ "High",
                                      PP_category %in% categories_special & Predicted_by == "Composition" ~ "Low", # cp32 and SSU5 super 
                                      size > `10% mean size (bp)` + size_max | size < size_min -`10% mean size (bp)` ~ "Low",
                                      T ~ "Medium")) %>%
      ...
      FINAL_PREDICTION = bind_rows(FINAL_PREDICTION_a, FINAL_PREDICTION_b) %>%
        ...
        mutate(Confidence = case_when(PP_category %in% categories_special & Predicted_by == "Composition" & # cp32 and SSU5 super
                                        size < `10% mean size (bp)` + size_max & size > size_min - `10% mean size (bp)` ~ "Medium",
                                      T ~ Confidence)) %>%
        
        select(genome_id, size, PP_category, Confidence, Predicted_by, cutoff_MinProteins, n_hits_MinProteins, Composition, composition_size, n_hits_composition) %>%
        rename(`Genome ID` = genome_id, `P-P type` = PP_category, `Confidence level` = Confidence, `Predicted by` = Predicted_by, `Genome size (bp)` = size, 
               `MinProteins cutoff` = cutoff_MinProteins, `MinProteins hits` = n_hits_MinProteins, `Composition size` = composition_size, `Composition hits` = n_hits_composition)
    } else {
      
      FINAL_PREDICTION = data.frame(`Genome ID` = "NA", `P-P type` = "NA", `Confidence level` = "NA", `Predicted by` = "NA", 
                                    `Genome size (bp)` = 0, `MinProteins cutoff` = 0, `MinProteins hits` = 0, `Composition size` = 0, `Composition hits` = 0)[-1, ]
    }
